...
HELP_TEXT = '''
py2so is a Cross-Platform, Fast and Flexible tool to compile the .py to .so(Linux and Mac) or .pdy(Win).
You can use it to hide the source code of py
It can be called by the main func as "from module import * "
py2so can be used in the environment of python2 or python3
...
Options:
  ...
  -r   --release    Release Mode, clear all the tmp files, only output the dist, Default: False


Example:
  python py2so.py -f test.py
  python py2so.py -f example/test1.py -r
  python py2so.py -d example/ -m test1.py,bbb/

  # some OS use command "python3" to run python3, like Ubuntu, you can use -p to solve it
  python3 py2so.py -p 3 -d example/
'''

buildingScript_fileName = 'build_script.py'
buildingScript_template = r"""# -*- coding: utf-8 -*-
...
setup(
    ...
    zip_safe=False,
    ext_modules = cythonize(extentions, nthreads=%s, build_dir="build_c", quiet=%s, compiler_directives=compiler_directives)
)
"""
...
def getCommandOptions(opts):
    ...
    for key, value in options:
        if key in ["-h", "--help"]:
            ...
            print("py2so version {0}".format('.'.join(py2so_version)))
            sys.exit(0)
            ...
            for path_assign in value.split(","):
                if path_assign[-1:] not in ['/', '\\']:  # if last char is not a path sep, consider it's assign a file
...
def genSetup(opts, will_compile_files):
    ...
    template = buildingScript_template
...
def clearBuildFolders():
    if os.path.isdir("build"):
        shutil.rmtree("build")
    if os.path.isdir("build_c"):
        shutil.rmtree("build_c")
...
def clearTmpFiles():
    ...
    if os.path.isdir("build_c"):
        shutil.rmtree("build_c")
    if os.path.isfile(buildingScript_fileName):
        os.remove(buildingScript_fileName)
...
def pyEncrypt(opts):
    ...
    if opts.quiet == "True":
        ...
        log = ""
    cmd = " {0} build_ext {1} --parallel=8".format(buildingScript_fileName, log)
    if opts.pyVer == '':
        cmd = 'python' + cmd
    else:
...
def genProject(opts, will_compile_files):
    ...
    for file in getFiles_inDir('build', True, 1, ['.so', '.pyd']):
        ...
        dest_path = os.path.join('dist', mid_path, file_name)
    ...
    for not_compile_file in not_compile_files:
        dest_path = os.path.join('dist', not_compile_file)
